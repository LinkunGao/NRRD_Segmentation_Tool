...
import {getClosestNipple} from "../../tools";
export class PanelOperationManager{
    ...
    setSlicePrams(currentSlice:any){
        ...
        this.slicePrams.currentSliceIndex = Math.ceil(currentSlice.index/currentSlice.RSARatio);
    ...
    start(){
        if(!this.started){
            ...
            this.operator.addEventListener("mousedown",this.dragPrams.handleOnDragMouseDown)
            this.operator.addEventListener("mouseup",this.dragPrams.handleOnDragMouseUp)
            this.started = true;
        } 
    }
    
    dispose(){
        this.operator.removeEventListener("mousedown",this.dragPrams.handleOnDragMouseDown)
        this.operator.removeEventListener("mousemove",this.dragPrams.handleOnDragMouseMove)
        this.operator.removeEventListener("mouseup",this.dragPrams.handleOnDragMouseUp)
        this.started = false;
    }
    ...
    private handleMouseDown(){
        ...
        return (ev:MouseEvent)=>{
            ...
            if(ev.button === 0){
                ...
                this.operator.addEventListener("mousemove",this.dragPrams.handleOnDragMouseMove);
            }
        }
...
export function valideClock(start:boolean,copperScene:Copper.copperScene, bg:HTMLElement, tl?:any,tr?:any,nrrdMesh?:any){
    let oldPoint:any;
